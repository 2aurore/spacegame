...
public class GamePlayingPanel extends JPanel implements Runnable {
	...
	private ImageIcon[] image_counting = new ImageIcon[2];	// ready go 2개의 배열 생성
...
//	Font font_score = new Font("Verdana", Font.BOLD, 140); //임시설정
	...
	private String mode = new A_MainFrame().getMode();
	...
	public GamePlayingPanel(String mode) {
		...
		for (int i = 0; i < image_counting.length; i++) {
			image_counting[i]= new ImageIcon("source/counting/count_"+i+".png");	//ready / go  카운트 이미지
			label_startCount[i] = new JLabel(image_counting[i]);
			label_startCount[i].setFont(Main.font_score);
		...
		label_score.setFont(Main.font_hp);
...
//		add(label_mode);
	...
	public void resultScreen() {
		...
		label_resultScore.setFont(Main.font_score);
		...
		label_gameOver.setFont(Main.font_score);
		...
		btn_conform.addActionListener( new ActionListener() {		
			...
			public void actionPerformed(ActionEvent e) {
				if(!game.isPlayerAlive()) 
					//new A_MainFrame().nextPanel(false);
				...
				if( character_y+170 < tempItem.getY()-48 && character_y > tempItem.getY()-48 || 
					...
					if(character_x < tempItem.getX()-48 && character_x+50 >tempItem.getX()-48 || 
						...
						if(tempItem.getType()==1 && !game.isInvincibility()) { //아이템 리스트에 따른 처리 1신발(빨라지기) 2보석(점수 증가) 3포션(hp 회복) 
							...
							if(game.getPlayer_HP()>100) game.setPlayer_HP(100);
	...
	void pauseScreen() {
		...
		label_pause.setFont(Main.font_score);
		...
		label_pauseInfo.setFont(Main.font_hp);
